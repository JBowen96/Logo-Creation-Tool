...
inquirer
    .prompt([
        ...
        },
        {
            type: 'input',
            name: 'textColor',
            message: 'Enter a color hex or name:',
            validate: (input) => {
        ...
        },
        {
            type: 'list',
            name: 'shape',
            message: 'Circle, Square, or Trianle?:',
            choices: ['Circle', 'Triangle', 'Square'],
        },
        {
            type: 'input',
            name: 'shapeColor',
            message: 'Enter a shape color (hexadecimal or keyword):',
            validate: (input) => {
                const isColorName = colorsArray.includes(input.toLowerCase());
                const isHexCode = /^#[0-9A-F]{6}$/i.test(input);
                return isColorName || isHexCode;
    ...
    .then((answers) => {
        ...
        // code for adjusting shape dimensions
        let fontSize;
